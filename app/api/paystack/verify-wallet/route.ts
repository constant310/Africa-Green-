...
export async function POST(req:NextRequest){
 try{
  ...
  const p=await fetch(`https://api.paystack.co/transaction/verify/${encodeURIComponent(reference)}`,{headers:{Authorization:`Bearer ${secret}`},cache:'no-store'});
  ...
  const result=await serviceRpc('credit_member_wallet_v4',{p_user:user.id,p_reference:reference,p_amount_kobo:Number(d.amount||0),p_provider_payload_hash:payloadHash});
