...
async function serviceRpc(name:string,args:Record<string,unknown>){
 const key=process.env.V4_SUPABASE_SERVICE_ROLE_KEY||process.env.SUPABASE_SERVICE_ROLE_KEY;
...
export async function POST(req:NextRequest){
 try{
  ...
  const matches=(Array.isArray(listed.data)?listed.data:[]).filter((d:any)=>String(d?.reference||'').startsWith('AOD-WALLET-')&&String(d?.status||'')==='success'&&String(d?.metadata?.purpose||'').toUpperCase()==='WALLET'&&String(d?.metadata?.user_id||'')===String(user.id));
  let credited=0;let alreadyProcessed=0;const reconciled:string[]=[];
  for(const d of matches){
   const reference=String(d.reference||'');const amount=Number(d.amount||0);if(!reference||amount<=0)continue;
   const payloadHash=crypto.createHash('sha256').update(JSON.stringify(d)).digest('hex');
   const result=await serviceRpc('credit_member_wallet_v4',{p_user:user.id,p_reference:reference,p_amount_kobo:amount,p_provider_payload_hash:payloadHash});
   if(result?.already_processed)alreadyProcessed+=1;else credited+=1;reconciled.push(reference);
  }
